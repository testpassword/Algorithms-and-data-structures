...
т.е. оно есть число O(log(n)). Каждый узел дерева будет хранить номер убийцы, и номер убиваемого. 
При модификации будем получать номер нового убиваемого, и изменять узлы дерева. 
